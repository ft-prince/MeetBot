...
const ZOOM_URL  = /^https?:\/\/[a-z0-9.-]*zoom\.us\/(j|wc\/join)\/\d+/i;
const TEAMS_URL = /^https?:\/\/(teams\.microsoft\.com|teams\.live\.com)\//i;
...
export function validateScheduleInput(input: Partial<ScheduleInput>): string | null {
  ...
  if (!input.meetingUrl || (!MEET_URL.test(input.meetingUrl) && !ZOOM_URL.test(input.meetingUrl) && !TEAMS_URL.test(input.meetingUrl))) {
