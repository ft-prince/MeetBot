...
function Accordion({ title, icon, badge, defaultOpen = false, accent = false, children }: {
  title: string; icon: string; badge?: string | number; defaultOpen?: boolean; accent?: boolean; children: React.ReactNode
}) {
  ...
  return (
    <div className="card overflow-hidden">
      <button
        onClick={() => setOpen(o => !o)}
        className={"w-full px-5 py-3.5 flex items-center gap-2 text-left transition-colors " + (accent ? "bg-accent-light" : "bg-app-bg") + " border-b " + (open ? "border-gray-200" : "border-transparent") + " hover:brightness-95"}
      >
    ...
    <>
      ...
      <div className="p-4 sm:p-8 flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          {/* LEFT — Transcript */}
          <div className="card overflow-hidden flex flex-col max-h-[60vh] lg:max-h-[70vh] lg:sticky lg:top-20">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Accordion icon="✍" title="Detailed Meeting Rewrite" defaultOpen={true} accent>
              {loading ? (
                ...
                <p className="text-sm leading-relaxed text-gray-700 whitespace-pre-line">{summary.detailedRewrite}</p>
              ) : (
                <p className="text-muted text-sm">{isLive ? "Will be generated once the meeting ends." : "Not yet available."}</p>
              )}
            </Accordion>
            ...
            <Accordion icon="🤖" title="Executive Summary" defaultOpen={true}>
              {loading ? (
                ...
                <p className="text-muted text-sm">{isLive ? "Summary will be generated after the meeting ends." : "Not yet available."}</p>
              )}
            </Accordion>

            <Accordion icon="💡" title="Key Insights &amp; Action Items" badge={summary?.keyInsights?.length || 0} defaultOpen={true}>
              {loading ? (
                <p className="text-muted text-sm">Loading…</p>
            ...
            <Accordion icon="📌" title="Important Points" badge={summary?.importantPoints?.length || 0} defaultOpen={false}>
              {loading ? (
                <p className="text-muted text-sm">Loading…</p>
